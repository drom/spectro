...
const cmdGen = (fmin, fmax, samples) => {
  ...
  const step = ('00000000' + (((span / samples) / 5) |0).toString()).slice(-8);
...
window.fmin = (val) => {
  state.fmin = val;
  return cmdGen(state.fmin, state.fmax, state.samples);
};
...
window.center = (fcenter) => {
  const fspan = state.fmax - state.fmin;
  state.fmin = fcenter - fspan / 2;
  state.fmax = fcenter + fspan / 2;
  return cmdGen(state.fmin, state.fmax, state.samples);
};
